      ...
      <a class="nav-link pr-0" href="#" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <div class="media align-items-center">
          <span class="img-thumbnail" style="border: 0 none; box-shadow: none;">
            <img src="https://flagcdn.com/32x24/{{ $BLang::getActiveLang() }}.png" alt="">
          </span>
          <div class="media-body  ml-2  d-none d-lg-block">
            <span class="mb-0 text-sm  font-weight-bold">
              {{ $BLang::get($BLang::getActiveLang()) }}
            </span>
          </div>
        </div>
      </a>
      <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
        @foreach($BLang::getAll() as $key => $value)
      ...
      <li class="nav-item dropdown" style="width: auto;">
        ...
        <div class="dropdown-menu dropdown-menu-right">
          ...
          <i class='bx bx-user' ></i>
            <span>{!! $BLang::get('account_page') !!}</span>
          </a>
          <a href="{{ route('billing.balance') }}" class="dropdown-item">
            <i class='bx bx-wallet'></i>
              <span>{!! $BLang::get('billing_balance') !!}</span>
            </a>
          <a href="/account/api" class="dropdown-item">
          <i class='bx bx-code-alt' ></i>
            <span>{!! $BLang::get('account_api') !!}</span>
          </a>
          @if(Auth::user()->root_admin)
          <a href="/admin" class="dropdown-item">
           <i class='bx bx-key'></i>
            <span>{!! $BLang::get('admin_area') !!}</span>
          </a>
          @endif
