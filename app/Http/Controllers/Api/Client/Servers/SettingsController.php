...
class SettingsController extends ClientApiController
{
    ...
    public function rename(RenameServerRequest $request, Server $server)
    {
        ...
        activity()
            ...
            ->log($user->name_first.' '.$user->name_last.' has Renamed the server to "'.$request->input('name').'"');
    ...
    public function reinstall(ReinstallServerRequest $request, Server $server)
    {
        ...
        activity()
            ...
            ->log($user->name_first.' '.$user->name_last.' has Reinstalled the server');
    ...
    public function dockerImage(SetDockerImageRequest $request, Server $server)
    {
        ...
        activity()
            ...
            ->withProperties([
                'serverID' => $server->id, 
                'module' => 'Server Settings', 
                'type' => 'Change dockerImage',
                'status' => 'Warning'
            ])
            ->log($user->name_first.' '.$user->name_last.' has Change the dockerImage to "'.$request->input('docker_image').'".');
